    ...
    handleSubmit = (e) => {
        ...
        this.setState({
            ...
            photograph:{
                ...
                file:URL.createObjectURL(e.target.file.files[0])
            }
            },()=>{
                let userDataParsed = JSON.stringify(this.state)
                localStorage.setItem('userData', userDataParsed);
                console.log('A name was submitted: ' , localStorage.getItem('userData'));
                // return (<Navigate to="/user/show" />)
                // navigate("/user/show");
                this.props.navigate('/user/show')
            }
        )
        

            // navigate('/user/show');
            //   this.navigation.navigate('user/show')
        // return <Navigate to='user/show' />
